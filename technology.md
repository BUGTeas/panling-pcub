...
如果您希望将此套件用在非插件平台上（例如 Fabric 等模组端），则需要自行构建魔改版的 Geyser (源码获取见使用说明)，且只能使用 `plugins/Geyser-Spigot` 文件夹中的内容，这里包含了 Geyser 所有版本通用的配置文件及资源包，除此之外的其它功能（包括数据包）均只为插件端设计，也就是说在这些平台上无法实现菜单书、交易、投掷限制等部分针对基岩版的修复或优化。
