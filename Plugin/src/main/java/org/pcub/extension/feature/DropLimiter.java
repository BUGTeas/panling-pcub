...
public class DropLimiter {
    ...
    public State check(Player player, String playerName, ItemStack usedItem, boolean isBedrock, boolean isTouch) {
        State limited = State.SUCCESS;
        boolean needCancel = dropLimit.get(player) >= 1;
        ...
        if(needCancel && dropLastAmount != dropAmount) {
            limited = State.LIMIT;
            if (common.debug) common.debugLogger(playerName + " 取消 - 投掷限制");
        }
        // 更新当前使用物品叠放量
        ...
        int enableContinuous = common.getScore("pcub_enable_continuous", playerName),dropSpeed = 7;
        if (
            // 始终禁用
            enableContinuous == 0 ||
            // 仅基岩版禁用
        ...
        // 投掷速度单位为刻数/次
        else dropSpeed = common.getScore("pcub_drop_interval", playerName);
